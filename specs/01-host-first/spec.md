...
**2026-05-21** — Discovered during VM testing that the wheel must NOT bundle the upstream `python/` package. `python.helpers.files.get_base_dir()` resolves siblings via `Path(__file__).parent.parent`; if installed under site-packages, that lands at `site-packages/`, which has no `conf/`/`prompts/`/`agents/`. Daemon raised `FileNotFoundError` on first request. Resolution: `pyproject.toml`'s `packages.find` now excludes `python*`, and the install path puts the cloned repo on `sys.path` via a `.pth` file. The clone is therefore load-bearing — it's not an artifact of how install.sh happens to work, it's part of how the runtime locates its assets. Full design captured in spec 07 D4. The D9 wrapper architecture stands; this is a refinement of what counts as "in the wrapper" vs "in the source clone".
